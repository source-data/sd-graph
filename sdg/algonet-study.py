...
# TODO: [x] try in neo4j louvain and betweenness and then inter community bridges
# TODO: [x] give name to cluster based on top TF IDF of concatenated titles.
# TODO: [ ] empirical p-value based on whole network distribution of centrality
# TODO: [x] make movie of percolation centrality
...
MATCH
    ...
    (coll:SDCollection),
    (coll)-->(a:SDArticle)-[r1:HasH]->(source),
    (coll)-->(b:SDArticle)-[r2:HasH]->(target)
WHERE
    ...
    // AND r1.rank <= 4 AND r2.rank <= 4  // dominant hypotheses
...
WHERE
    ...
    // AND r.rank <= 5  // dominant hypotheses
    AND (NOT h.self_test)
...
WITH DISTINCT
    ...
    COUNT(DISTINCT a) AS n_articles
RETURN DISTINCT {
    source: {id: id(source), description: source.name, dois: dois, aggregated_pub_dates: aggregated_pub_dates, n_articles: n_articles},
    target: {id: id(target), description: target.name, dois: dois, aggregated_pub_dates: aggregated_pub_dates, n_articles: n_articles}
} AS edge
    '''
    returns = ['edge']
...
RETURN DISTINCT
    {
        source: {id: source.concept_id, description: source.concept_name, dois: [], aggregated_pub_dates: aggregated_pub_dates, n_panels: n_panels, n_articles: n_articles},
        target: {id: target.concept_id, description: target.concept_name, dois: [], aggregated_pub_dates: aggregated_pub_dates, n_panels: n_panels, n_articles: n_articles}
    } AS edge
    '''
    returns = ['edge']


class ABSTRACTS_BY_HYP_COMMUNITY(Query):
    code = '''
MATCH (h:H_Entity)
...
WITH
  COLLECT(DISTINCT h) AS module, 
  COLLECT(DISTINCT h.name) AS components,
...
WITH DISTINCT 
  ...
  community_id,
  components,
  COLLECT(DISTINCT h.name) AS overlap_with_module, 
...
RETURN
  COLLECT(DISTINCT a.title) as abstracts,
  components,
  community_id;
    '''
    returns = ['abstracts', 'community_id', 'components']


class ALL_ABSTRACTS(Query):
    code = '''
MATCH 
    (coll:SDCollection)-->(a:SDArticle)-[r:HasH]->(h:Hypothesis)
WHERE
...
def full_graph(q):
    ...
    for r in results:
        ...
        g.add_node(source['id'], description=source['description'], dois=source.get('dois', []), aggregated_pub_dates=source.get('aggregated_pub_dates'), n_panels=source.get('n_panels'), n_articles=source.get('n_articles'))
        g.add_node(target['id'], description=target['description'], dois=target.get('dois', []), aggregated_pub_dates=target.get('aggregated_pub_dates'), n_panels=target.get('n_panels'), n_articles=target.get('n_articles'))
...
def list_subgraph_central(
    ...
    **kwargs
    ...
    for i, subg in enumerate(subgraphs[:max]):
        ...
        # centrality = {k: v/subg.nodes[k]['n_articles'] for k, v in centrality.items() if not isnan(v)}
...
def name_hypothesis_community(limit_date):
    def print_best(abstracts, X, keywords, components):
        keywords = vectorizer.get_feature_names()
        N, M = X.shape
        X = X.toarray()
        community_ids = list(components.keys())
        for i in range(N):
            ...
            topics = [t for t in topics if t not in components[community_id]]
            print(f"This collection is about: {', '.join(topics[:5])}")
            print(f"Key bio-entities: {', '.join(components[community_id])}")
            print()

    results_corpus = DB.query(ALL_ABSTRACTS(params={'date': limit_date}))
    corpus = [r['abstract'] for r in results_corpus]
    results_modules = DB.query(ABSTRACTS_BY_HYP_COMMUNITY())
    modules = {r['community_id']: r['abstracts'] for r in results_modules}
    module_components = {r['community_id']: r['components'] for r in results_modules}
    ...
    for community_id, abstracts in modules.items():
        concatenated_abstracts.append(' '.join([a for a in abstracts]))
    X_community = vectorizer.transform(concatenated_abstracts)
    print_best(concatenated_abstracts, X_community, keywords, module_components)
...
def automagic(g, community_funct, highlight_funct, stats, viz=True):
    ...
    general_highlights = list_subgraph_central([gcc], stats, funct=highlight_funct, viz=viz)
    ...
    community_highlights = list_subgraph_central(subgraphs_hyp, stats, funct=highlight_funct, viz=viz)
...
def neo2nx(viz=False):
    ...
    # def highlight_funct(x): return nx.eigenvector_centrality(x, tol=1E-04)
    # def highlight_funct(x): return curvature(x)
    ...
    general_highlights, community_highlights = automagic(g_entity_as_nodes, community_funct, highlight_funct_e, stats, viz=viz)
