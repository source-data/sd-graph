...
def create_graph(collection_name):
    ...
    for doi in sdapi.doi_list:
        ...
        if doi is None:
            print(f"paper with no doi could not be retrieved")
        else:
            a = sdapi.article(doi)
            if a is None:
                print(f"paper with doi={doi} could not be retrieved")
                ...
                for fig_id in a.children:
                    ...
                    if f is None:
                        print(f"figure {fig_id} from doi={doi} could not be retrieved")
                        ...
                        for panel_id in f.children:
                            ...
                            if p is None:
                                print(f"panel {panel_id} could not be retrieved")
